...
-   `pjlink_device_info` (info metric, labels: `name`, `manufacturer`, `product_name`, `pjlink_class`): Device identity reported over PJLink (`NAME`/`INF1`/`INF2`/`CLSS`). Registered as `pjlink_device` in code — `InfoMetricFamily` appends the `_info` suffix in the exposition format.
